...
| Platform export candidates | `platform-assets/ios/`, `platform-assets/android/`, `platform-assets/splash/` | `scripts/exportPlatformAssets.mjs`와 macOS `sips`로 후보 생성 | 제출 후보. final art/legal/device 확인 필요 |
| Android native launcher candidates | `android/app/src/main/res/mipmap-*` | `npm run export:assets`가 source icon에서 resize | Android shell inspection용 후보 |
...
- 현재 자산은 release candidate용 직접 제작 SVG auxiliary pack + generated v2 raster core art pack + RC-13 platform export 후보이다. 실제 스토어 제출 전 commissioned/final art 소유권, adaptive icon foreground/background, splash image, feature graphic을 확정해야 한다.
...
- 네이티브 icon/splash export는 `NATIVE_BUILD_GUIDE.md`와 `APP_ICON_SPLASH_EXPORT.md`를 따른다.
