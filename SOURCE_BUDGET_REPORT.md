...
| 항목 | LOC/bytes | 감사 판정 |
| --- | ---: | --- |
| 순수 handwritten runtime 구현 `src/app src/core src/game src/state src/systems src/ui` | 6,918 LOC | 실제 gameplay/UI/system 구현 규모 |
| 순수 handwritten unit/E2E tests `src/tests e2e` excluding generated matrix | 2,285 LOC | 실제 사람이 작성한 검증 규모 |
| 순수 handwritten gameplay/UI/system/test 합계 | 9,203 LOC | 이번 감사의 기준값 |
...
| handwritten runtime+test byte size | 323,406 bytes | docs/build/assets 제외 기준 |
...
순수 handwritten 구현은 약 6.9K LOC, handwritten 검증까지 합치면 약 9.2K LOC다. 이번 v2 raster/HUD art pass와 RC-4 interaction polish에서 SVG와 PNG 산출물은 직접 관리 가능한 asset pack으로 유지되지만, generated/asset 파일 용량 자체를 구현 규모 근거로 쓰지 않는다. 남은 리스크는 commissioned/final art ownership, 실제 파일 기반 사운드, 실제 광고/IAP SDK, 물리 기기 QA 쪽으로 분리한다.
