...
find src/assets/raster -type f -name '*.png' | wc -l
find src/assets/raster -type f -name '*.png' | sort | xargs du -ch | tail -1
...
| 항목 | LOC/bytes | 감사 판정 |
| --- | ---: | --- |
| 순수 handwritten runtime 구현 `src/app src/core src/game src/state src/systems src/ui` | 9,388 LOC | 실제 gameplay/UI/system 구현 규모. RC-8 storage/WebView readiness 포함 |
| 순수 handwritten unit/E2E tests `src/tests e2e` excluding generated matrix | 3,235 LOC | 실제 사람이 작성한 검증 규모. RC-8 stress/E2E 포함 |
| 순수 handwritten gameplay/UI/system/test 합계 | 12,623 LOC | 이번 감사의 기준값 |
...
| handwritten runtime+test byte size | 439,730 bytes | docs/build/assets 제외 기준 |
| runtime dist PNG payload | 12 files / 14M | release-only raster candidates excluded from runtime registry |
...
| 수제 SVG 파일 | 253개 | icons 61, items 158, mascots 5, portraits 8, release/final candidates 16, tiers 5 |
| Raster PNG 파일 | 15개 | home 2, companions 8, release 5. stale source working copies removed |
| registry asset key | 242개 | `assetRegistryMatrix.test.ts`가 registry 구조와 SVG 무결성 검증 |

## 판단

순수 handwritten 구현은 약 9.4K LOC, handwritten 검증까지 합치면 약 12.6K LOC다. 이번 v2 raster/HUD art pass, RC-4 interaction polish, RC-5 CSS component system pass, RC-6 product feel pass, RC-7 retention systems pass, RC-8 release bug bash에서 SVG와 PNG 산출물은 직접 관리 가능한 asset pack으로 유지되지만, generated/asset 파일 용량 자체를 구현 규모 근거로 쓰지 않는다. 남은 리스크는 commissioned/final art ownership, 실제 파일 기반 사운드, 실제 광고/IAP SDK, 서버 검증 calendar/push notification, 물리 기기 QA 쪽으로 분리한다.
