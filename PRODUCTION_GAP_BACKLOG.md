...
| 어설픈 CSS override 누적 위험 | 화면이 부분별로 덧칠된 대학생 PPT/양산형 앱처럼 보일 수 있음 | 첫 raster pass 실패 후 RC-4에서 wood/parchment/orange game HUD skin으로 재정리했고, RC-5에서 `layout.css`를 import manifest로 축소한 뒤 `shell/hud/screens/effects`와 `.ui-*` skin classes로 안정화 | 해결 |
...
| RC-10 upgrade quick-buy / shelf product quality | RC-10 구현 후에도 360/390 screenshot에서 긴 정보 카드와 CTA/tab 밀도, tool pedestal 빈 슬롯 느낌이 남아 8.0 product gate 미달 | RC-11에서 작업대 레버 장치, filled pedestal, first card hierarchy, 360px focused shelf capture로 보강. `RC11_INDEPENDENT_RESCORE.md` 8.1 | 해결 |
| RC-10 store screenshot product framing | public/internal wording은 정리됐지만 gameplay panel이 작고 약해 strong key art에 앱 캡처를 얹은 느낌이 남음 | RC-11에서 phone panel scale/crop, upgrade shelf scroll framing, modal reward close framing, Korean copy width/line-break 재조정. `RC11_INDEPENDENT_RESCORE.md` 8.1 | 해결 |
...
재감사, reward hardening, playtest/balance/bug bash, direct art/CSS pass, 첫 raster pass 실패 재분류, v2 raster/HUD art pass, RC-4 Game UI Skin & Interaction Polish, RC-5 CSS Debt & Component System Pass, RC-6 product feel pass, RC-7 retention systems pass, RC-8 release bug bash 중 발견한 기술/기능 내부 P1은 코드/CSS/테스트/E2E로 수정했다. 특히 업그레이드 card-list P1, 설정 form smell, save util dialog smell은 `RC4_UI_SKIN_AUDIT.md` 기준으로 개선했고, CSS override debt는 `RC5_CSS_COMPONENT_AUDIT.md` 기준으로 P0/P1 없이 정리했다. D1/D3/D7 복귀 보상과 첫 환생 이후 목표 chain도 RC-7에서 실제 시스템으로 구현했다. RC-10 integrity pass에서 product-quality gate를 combined 7.7 no-go로 보정한 기록은 유지한다. RC-11에서 남은 upgrade quick-buy/shelf와 store screenshot framing P1을 해결했고, `RC11_INDEPENDENT_RESCORE.md` 기준 combined 8.1이다. RC-13에서 Android native shell과 platform asset 후보, final layout/store guards를 추가했다. 현재 남은 항목은 P2/P3 또는 외부 제출 준비 항목이다.
