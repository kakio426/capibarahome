  ...
  return (
    <main className="screen stack-screen">
      ...
      <Panel className="settings-actions">
        <Button variant="secondary" onClick={() => updateState((current) => restartTutorial(current))}>튜토리얼 다시 보기</Button>
        <Button variant="secondary" onClick={() => setSaveOpen(true)}>세이브 Export/Import</Button>
        <Button variant="secondary" onClick={() => GameActions.save()}>강제 저장</Button>
        <Button variant="danger" onClick={() => setResetOpen(true)}>저장 데이터 초기화</Button>
      </Panel>
