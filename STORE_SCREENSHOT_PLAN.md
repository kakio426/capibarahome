...
| `store-screenshots/google-play-feature-graphic.png` | Google Play 후보 | feature graphic | key art crop | RC-14 검증 완료 |

## 사용 제한

- 현재 이미지는 store submission 후보 pack이지만 최종 제출 전 실제 iOS/Android simulator 또는 물리 기기에서 다시 캡처해야 한다.
- 현재 후보 치수는 iPhone `1290 x 2796`, Android `1080 x 1920`, feature graphic `1024 x 500`이다.
- 공개 스토어 스크린샷에는 구현 방식, 검증 방식, 비공개 준비 상태 문구를 넣지 않는다.
- RC-14에서 Korean copy 금지어, text clipping, screenshot file size, PNG dimension guard, feature graphic dimension guard를 자동화했다. 최종 제출 전에는 실제 store 기기 프레임, 플랫폼별 해상도, final exported icon/splash 기준으로 다시 캡처한다.
